...
export async function POST(request: Request) {
    try {
        ...
        const participant = await registerParticipantToEvent(
            session.user.id,
            body.eventId
        )
        return NextResponse.json(
            {
                success: true,
                message: "Participant registered to event",
                data: {
                    id: participant.id,
        ...
        if (
            ...
            error.message === "PARTICIPANT_NOT_FOUND"
        ) {
            return NextResponse.json(
                {
                    success: false,
        ...
        return NextResponse.json(
            {
                success: false,
                message: "Gagal mendaftarkan peserta ke event",
            },
            {
                status: 500,
            },
